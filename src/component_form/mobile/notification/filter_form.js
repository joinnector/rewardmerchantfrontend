...
const NotificationFilterForm = (props) => {
	...
	const filter_list = (values) => {
		...
		props.history.replace(`/notification-list?${params.toString()}`);
		props.api_merchant_list_notifications(values);
	};

	return (
		<antd.Layout className="generic-form">
			<antd.Card style={{ minWidth: 400, maxWidth: 450, }} bordered={false} title={"FILTER NOTIFICATION"}>
				<antd.Form form={form} onFinish={filter_list}>
					...
					<antd.Form.Item label="Parent type" initialValue={collection_helper.process_url_params(props.location.search).get("parent_type", "")} name="parent_type" rules={[{ required: false, message: "Please select a value" }]} hasFeedback labelCol={{ span: 24 }} wrapperCol={{ span: 24 }}>
						<antd.Select >
							{(props.systeminfos && props.systeminfos.notification_parent || []).map(type => <antd.Select.Option key={type} value={type}>{collection_helper.get_lodash().capitalize(type)}</antd.Select.Option>)}
						</antd.Select>
					</antd.Form.Item>

					<antd.Form.Item label="Event" initialValue={collection_helper.process_url_params(props.location.search).get("event", "")} name="event" rules={[{ type: "string", max: 100, message: "Name must be between 1 - 100 chars" }, { required: false, message: "Please select a value" }]} hasFeedback labelCol={{ span: 24 }} wrapperCol={{ span: 24 }}>
						<antd.Input />
					</antd.Form.Item>
