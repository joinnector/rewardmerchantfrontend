...
const CouponCodeFilterForm = (props) => {
	...
	const filter_list = (values) => {
		...
		props.history.replace(`/couponcode-list?${params.toString()}`);
		props.api_merchant_list_couponcodes(values);
	};

	return (
		<antd.Layout className="generic-form">
			<antd.Card style={{ minWidth: 400, maxWidth: 450, }} bordered={false} title={"FILTER COUPONCODE"}>
				<antd.Form form={form} onFinish={filter_list}>
					...
					<antd.Form.Item label="Is available" initialValue={collection_helper.process_url_params(props.location.search).get("is_available", false)} name="is_available" valuePropName="checked" rules={[{ required: false }]} labelCol={{ span: 24 }} wrapperCol={{ span: 24 }}>
...
export default CouponCodeFilterForm;