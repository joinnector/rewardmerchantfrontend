...
const CouponFilterForm = (props) => {
	...
	const filter_list = (values) => {
		...
		props.history.replace(`/coupon-list?${params.toString()}`);
		props.api_merchant_list_coupons(values);
	};

	return (
		<antd.Layout className="generic-form">
			<antd.Card style={{ minWidth: 400, maxWidth: 450, }} bordered={false} title={"FILTER COUPON"}>
