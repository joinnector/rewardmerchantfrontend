...
const TaskActivityFilterForm = (props) => {
	...
	const filter_list = (values) => {
		...
		props.history.replace(`/taskactivity-list?${params.toString()}`);
		props.api_merchant_list_taskactivities(values);
	};

	return (
		<antd.Layout className="generic-form">
			<antd.Card style={{ minWidth: 400, maxWidth: 450, }} bordered={false} title={"FILTER TASK ACTIVITY"}>
				<antd.Form form={form} onFinish={filter_list}>
					...
					<antd.Form.Item label="Task id" initialValue={collection_helper.process_url_params(props.location.search).get("task_id", "")} name="task_id" rules={[{ required: false, message: "Please enter a value" }]} hasFeedback labelCol={{ span: 24 }} wrapperCol={{ span: 24 }}>
						<antd.Input />
					</antd.Form.Item>
...
export default TaskActivityFilterForm;