...
import ViewForm from "../../../component_form/review/view_form";
import FilterForm from "../../../component_form/review/filter_form";
...
class ReviewListComponent extends React.Component {
	...
	// mounted
	componentDidMount() {
		this.api_merchant_list_reviews({ page: this.state.page, limit: this.state.limit, ...collection_helper.process_objectify_params(this.props.location.search) });
	}
	...
	api_merchant_get_reviews(values) {
		// eslint-disable-next-line no-unused-vars
		const opts = {
			event: constant_helper.get_app_constant().API_IGNORE_DISPATCH,
			endpoint: `api/open/reviews/${values._id}`,
			params: {},
	...
	process_list_data() {
		return (this.props.reviews && this.props.reviews.items || []).map(item => ({ ...item, key: item._id }));
	}
	...
	process_list_column() {
		return [
			...
			// eslint-disable-next-line no-unused-vars
			{ title: "Rating", dataIndex: "rating", key: "rating", render: (text, record) => <antd.Typography.Paragraph>{(record.rating ? collection_helper.get_lodash().capitalize(record.rating) : "-")}</antd.Typography.Paragraph> },
			// eslint-disable-next-line no-unused-vars
			{
				// eslint-disable-next-line no-unused-vars
	...
	render_drawer_action() {
		if (this.state.action === "view") {
			...
			return <FilterForm {...this.props} api_merchant_list_reviews={this.api_merchant_list_reviews} />;
		}
	}
			...
			<antd.Layout>
				<antd.PageHeader
					...
					<antd.Table
						...
						pagination={{
							onChange: (page) => {
								if (!this.state.loading) this.api_merchant_list_reviews({ page: page });
							},
...
ReviewListComponent.propTypes = properties;
