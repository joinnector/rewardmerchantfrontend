...
import ViewForm from "../../../component_form/stat/view_form";
import FilterForm from "../../../component_form/stat/filter_form";
...
const properties = {
	...
	stats: prop_types.object.isRequired,
...
//from app
class StatListComponent extends React.Component {
	constructor(props) {
		super(props);
		...
		this.api_merchant_get_stats = this.api_merchant_get_stats.bind(this);
		this.api_merchant_list_stats = this.api_merchant_list_stats.bind(this);
	...
	// mounted
	componentDidMount() {
		this.api_merchant_list_stats({ page: this.state.page, limit: this.state.limit, ...collection_helper.process_objectify_params(this.props.location.search) });
	}
	...
	api_merchant_get_stats(values) {
		// eslint-disable-next-line no-unused-vars
		const opts = {
			event: constant_helper.get_app_constant().API_IGNORE_DISPATCH,
			endpoint: `api/v1/merchant/stats/${values._id}`,
			params: {},
	...
	api_merchant_list_stats(values) {
		...
		// eslint-disable-next-line no-unused-vars
		const opts = {
			event: constant_helper.get_app_constant().API_MERCHANT_LIST_STAT_DISPATCH,
			endpoint: "api/v1/merchant/stats",
			has_authorization: true,
			params: {
				page: values.page || 1,
	...
	process_list_data() {
		return (this.props.stats && this.props.stats.items || []).map(item => ({ ...item, key: item._id }));
	}
	...
	process_list_column() {
		return [
			...
			// eslint-disable-next-line no-unused-vars
			{ title: "Wallet id", dataIndex: "wallet_id", key: "wallet_id", render: (text, record) => <antd.Typography.Paragraph>{record.wallet_id ? record.wallet_id : "-" }</antd.Typography.Paragraph> },
			// // eslint-disable-next-line no-unused-vars
			...
			// eslint-disable-next-line no-unused-vars
			{ title: "Amount", dataIndex: "amount", key: "amount", render: (text, record) => <antd.Typography.Paragraph>{Number(record.amount)} / {collection_helper.get_lodash().capitalize(record.currency_code)}</antd.Typography.Paragraph> },
			// eslint-disable-next-line no-unused-vars
			{ title: "Remarks", dataIndex: "remarks", key: "remarks", render: (text, record) => <antd.Typography.Paragraph>{(record.remarks ? collection_helper.get_lodash().capitalize(record.remarks) : "-")}</antd.Typography.Paragraph> },
			// eslint-disable-next-line no-unused-vars
			{
				// eslint-disable-next-line no-unused-vars
	...
	render_drawer_action() {
		if (this.state.action === "view") {
			...
			return <FilterForm {...this.props} api_merchant_list_stats={this.api_merchant_list_stats} />;
		}
	}
	...
	render() {
		...
		const count = (this.props.stats && this.props.stats.count || 0);
			...
			<antd.Layout>
				<antd.PageHeader
					...
					<antd.Table
						...
						pagination={{
							onChange: (page) => {
								if (!this.state.loading) this.api_merchant_list_stats({ page: page });
							},
...
StatListComponent.propTypes = properties;
