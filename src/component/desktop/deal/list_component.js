...
import CreateForm from "../../../component_form/deal/create_form";
import EditForm from "../../../component_form/deal/edit_form";
import ViewForm from "../../../component_form/deal/view_form";
import FilterForm from "../../../component_form/deal/filter_form";
...
class DealListComponent extends React.Component {
	constructor(props) {
		...
		this.api_merchant_create_deals = this.api_merchant_create_deals.bind(this);
		this.api_merchant_get_deals = this.api_merchant_get_deals.bind(this);
		this.api_merchant_update_deals = this.api_merchant_update_deals.bind(this);
		this.api_merchant_delete_deals = this.api_merchant_delete_deals.bind(this);
		...
		this.on_couponcode = this.on_couponcode.bind(this);
		this.on_coupon = this.on_coupon.bind(this);
		this.on_task = this.on_task.bind(this);
		this.on_review = this.on_review.bind(this);
	...
	// mounted
	componentDidMount() {
		this.api_merchant_list_deals({ page: this.state.page, limit: this.state.limit, ...collection_helper.process_objectify_params(this.props.location.search) });
	}
	...
	api_merchant_create_deals(values) {
		// eslint-disable-next-line no-unused-vars
		const opts = {
			event: constant_helper.get_app_constant().API_SUCCESS_DISPATCH,
			endpoint: "api/v1/merchant/deals",
			params: {},
	...
	api_merchant_update_deals(values) {
		// eslint-disable-next-line no-unused-vars
		const opts = {
			event: constant_helper.get_app_constant().API_SUCCESS_DISPATCH,
			endpoint: `api/v1/merchant/deals/${this.state.action_item._id}`,
			params: {},
			has_authorization: true,
			has_signature: true,
			attributes: {
	...
	api_merchant_delete_deals(values) {
		// eslint-disable-next-line no-unused-vars
		const opts = {
			event: constant_helper.get_app_constant().API_SUCCESS_DISPATCH,
			endpoint: `api/v1/merchant/deals/${values._id}`,
			params: {},
			has_authorization: true,
			headers: {
	...
	process_list_column() {
		const popover_content = (record) => {
			return (
				<div style={{ display: "block" }}>
					...
					<p><a onClick={() => this.on_couponcode(record)}>Coupon codes <antd_icons.ArrowRightOutlined/></a></p>
					...
					<p><a onClick={() => this.on_review(record)}>Reviews <antd_icons.ArrowRightOutlined/></a></p>
				</div>
			);
		};
		...
		return [
			...
			// eslint-disable-next-line no-unused-vars
			{ title: "Promo code", dataIndex: "promo_code", key: "promo_code", render: (text, record) => <antd.Typography.Paragraph>{record.promo_code ? record.promo_code : "-"}</antd.Typography.Paragraph> },
			// eslint-disable-next-line no-unused-vars
			{ title: "Sell price", dataIndex: "sell_price", key: "sell_price", render: (text, record) => <antd.Typography.Paragraph>{Number(record.sell_price)} / {collection_helper.get_lodash().capitalize(record.currency_code)}</antd.Typography.Paragraph> },
			// eslint-disable-next-line no-unused-vars
	...
	on_delete(record) {
		...
		antd.Modal.confirm({
			...
			onOk() {
				_this.api_merchant_delete_deals(record);
			}
		});
	}

	on_upload(record) {
		this.props.history.push(`/upload-list?parent_id=${record._id}&parent_type=deals`);
	}

	on_couponcode(record) {
		this.props.history.push(`/couponcode-list?deal_id=${record._id}`);
	}

	on_coupon(record) {
		this.props.history.push(`/coupon-list?deal_id=${record._id}`);
	}
	
	on_task(record) {
		this.props.history.push(`/task-list?deal_id=${record._id}&type=deal`);
	}

	on_review(record) {
		this.props.history.push(`/review-list?parent_id=${record._id}&parent_type=deals`);
	}
	...
	render_drawer_action() {
		if (this.state.action === "create") {
			return <CreateForm {...this.props} api_merchant_create_deals={this.api_merchant_create_deals} />;
		} else if (this.state.action === "edit") {
			return <EditForm {...this.props} action_item={this.state.action_item} api_merchant_update_deals={this.api_merchant_update_deals} />;
			...
			return <FilterForm {...this.props} api_merchant_list_deals={this.api_merchant_list_deals} />;
		}
	}
